...
export const runCpp = async (code, inputString = '') => {
  try {
    // JSCPP requires this specific version from unpkg or cdnjs
    await loadScript('https://cdnjs.cloudflare.com/ajax/libs/JSCPP/2.0.7/JSCPP.js');
  } catch (e) {
...
export const runOfflineCode = async (language, sourceCode, sampleInput) => {
  ...
  // Unsupported offline language - signal to fallback to AST simulator
  return null;
};
